...
for i = 1:length(t)
    ...
    %Now by mixing the transmit and receive, generate the beat signal by element-wise matrix multiplication of the tx/rx signals.
    Mix(i) = Tx(i) * Rx(i);
end

%Range measurement
%Reshape the vector into a Nr x Nd array. Nr and Nd here would also define the size of range and Doppler FFT respectively.
...
%Output of FFT is a double sided signal, getting one side of the spectrum. 
...
%CFAR implementation, sliding window through the complete range Doppler map
